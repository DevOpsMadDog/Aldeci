...
try:  # pragma: no cover - exercised implicitly by doc generation
    import structlog
except ModuleNotFoundError:  # pragma: no cover - fallback when optional dependency missing
    import logging

    _fallback_logger = logging.getLogger("explainability")

    if not _fallback_logger.handlers:
        logging.basicConfig(level=logging.INFO)

    class _ShimLogger:
        """Minimal structlog-compatible shim used during documentation builds."""
